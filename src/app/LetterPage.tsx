...
function DrawingCanvas({ letter }: { letter: LetterEntry }) {
  ...
  const clear = useCallback(() => {
    ...
    if (ctx && canvasRef.current) { ctx.clearRect(0,0,canvasRef.current.width,canvasRef.current.height); setIsDone(false); }
  }, []);
  ...
  const move = (x:number,y:number) => {
    ...
    ctx.strokeStyle=DK; ctx.lineWidth=3.5; ctx.lineCap="round"; ctx.lineJoin="round"; ctx.stroke();
    lastPos.current={x,y};
  };
  ...
  return (
    <div style={{ border:`1px solid ${BR}` }}>
      ...
      <div ref={containerRef} style={{ position:"relative", height:280, overflow:"hidden", backgroundImage:"repeating-linear-gradient(to bottom,transparent,transparent 27px,rgba(148,163,184,0.25) 27px,rgba(148,163,184,0.25) 28px)", backgroundSize:"100% 28px" }}>
        <div style={{ position:"absolute", inset:0, display:"flex", alignItems:"center", justifyContent:"center", pointerEvents:"none", zIndex:1, paddingBottom:28 }}>
          <span style={{ fontFamily:F.arabic, fontSize:160, color:"rgba(0,0,0,0.05)", lineHeight:1, direction:"rtl", userSelect:"none" }}>{letter.arabic}</span>
        </div>
...
function PracticeTab({ letter }: { letter: LetterEntry }) {
  return (
    <>
      <DrawingCanvas letter={letter} />
      <Card label="TIPS">
        <div style={{ fontFamily:F.mono, fontSize:10, color:O, letterSpacing:"0.15em", marginBottom:14 }}>FOR REAL PAPER PRACTICE</div>
        {PRACTICE_TIPS.map((tip,i) => (
          <div key={i} style={{ display:"flex", gap:14, marginBottom:14 }}>
            <span style={{ fontFamily:F.mono, fontSize:11, color:MU, flexShrink:0, paddingTop:1 }}>{String(i+1).padStart(2,"0")}</span>
            <p style={{ fontFamily:F.mono, fontSize:11, color:DK, lineHeight:1.75, margin:0 }}>{tip}</p>
          </div>
        ))}
      </Card>
    </>
...
export function LetterPage({ letter, group, onBack, prevLetter, nextLetter, onSelectLetter }: {
  letter: LetterEntry; group: LetterGroup; onBack: ()=>void;
  prevLetter: LetterEntry|null; nextLetter: LetterEntry|null;
  onSelectLetter: (l:LetterEntry)=>void;
}) {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ padding:"24px 20px 0", display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        ...
        <div style={{ width:80, height:80, border:`1px solid ${BR}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginLeft:16 }}>
          <span style={{ fontFamily:F.arabic, fontSize:40, color:DK }}>{letter.arabic}</span>
        </div>
      </div>
      <div style={{ display:"flex", borderBottom:`1px solid ${BR}`, margin:"20px 0 0" }}>
        {TABS.map((t) => {
          const active=activeTab===t.id;
          return <button key={t.id} onClick={()=>setActiveTab(t.id)} style={{ flex:1, padding:"12px 0", background:"none", border:"none", cursor:"pointer", fontFamily:F.mono, fontSize:11, letterSpacing:"0.1em", color:active?DK:MU, borderBottom:active?`2px solid ${DK}`:"2px solid transparent", marginBottom:-1 }}>{t.label}</button>;
        })}
      </div>
      <div style={{ padding:"20px", display:"flex", flexDirection:"column", gap:16 }}>
        {activeTab==="learn"    && <LearnTab letter={letter} />}
        {activeTab==="forms"    && <FormsTab letter={letter} />}
        {activeTab==="practice" && <PracticeTab letter={letter} />}
      </div>
      <div style={{ display:"flex", borderTop:`1px solid ${BR}`, marginTop:8 }}>
        <button onClick={()=>prevLetter&&onSelectLetter(prevLetter)} disabled={!prevLetter} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"14px 16px", background:"none", border:"none", borderRight:`1px solid ${BR}`, cursor:prevLetter?"pointer":"default", fontFamily:F.mono, fontSize:11, color:prevLetter?DK:MU }}>
          <ArrowRight size={13} style={{ transform:"rotate(180deg)" }} />PREV
        </button>
        <button onClick={()=>nextLetter&&onSelectLetter(nextLetter)} disabled={!nextLetter} style={{ flex:1, display:"flex", alignItems:"center", justifyContent:"center", gap:8, padding:"14px 16px", background:"none", border:"none", cursor:nextLetter?"pointer":"default", fontFamily:F.mono, fontSize:11, color:nextLetter?DK:MU }}>
          {nextLetter?<><span style={{fontFamily:F.ui}}>{nextLetter.name}</span><span style={{fontFamily:F.arabic,fontSize:16}}>({nextLetter.arabic})</span></>:"—"}
          <ArrowRight size={13}/>
        </button>
      </div>
      <div style={{ height:24 }} />
